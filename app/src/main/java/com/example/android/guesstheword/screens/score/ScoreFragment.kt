...
class ScoreFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // DONE (05) Create ScoreViewModel by using ViewModelProvider as usual, except also
        // pass in your ScoreViewModelFactory
        viewModelFactory = ScoreViewModelFactory(scoreFragmentArgs.score)
        viewModel = ViewModelProvider(this, viewModelFactory).get(ScoreViewModel::class.java)

        binding.playAgainButton.setOnClickListener { viewModel.onEventPlayAgain() }
        ...
        viewModel.eventPlayAgain.observe(viewLifecycleOwner, Observer { isEventHappen ->
            if (isEventHappen) {
                findNavController().navigate(ScoreFragmentDirections.actionRestart())
                viewModel.resetPlayAgain()
            }
        })
